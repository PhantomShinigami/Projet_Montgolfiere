...
const char* ssid = "CCF_Montgolfiere"; //wifi
const char* password = "";
...
void loop()
{
  ...
  float voltage= (sensorVal*3.3)/1024.0; //convertir le signal analogique 0-5Volt en signal 0-1024

  float pressure_pascal = (3.0*((float)voltage-0.47))*1000000.0; //convertir le signal en une pression pascal
  float pressure_bar = pressure_pascal/10e5 *0.83; //convertir la pression de pascals à bars
  
  
  char result[8];
  dtostrf(pressure_bar,6,2,result);
  ...
  client.publish("room/pression", result);
  ...
  //lcd.setCursor(5,0);
  Serial.print(pressure_bar); //écrire la variable entre les parenthèses
